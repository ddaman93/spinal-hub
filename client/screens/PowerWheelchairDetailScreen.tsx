...
import { getPowerChairById, POWER_CHAIR_BRAND_INFO } from "@/data/powerWheelchairs";

type RouteProps = RouteProp<MainStackParamList, "PowerWheelchairDetail">;

export default function PowerWheelchairDetailScreen() {
  ...
  const chair = getPowerChairById(params.productId);
  const brandInfo = chair
    ? POWER_CHAIR_BRAND_INFO.find((b) => b.id === chair.brand)
    : undefined;
      ...
      >
        ...
        <View style={styles.content}>
          ...
          {brandInfo && (
            <ThemedText
              type="caption"
              style={[styles.brandLabel, { color: theme.textSecondary }]}
            >
              {brandInfo.title}
            </ThemedText>
          )}
          ...
          {chair.productUrl ? (
            <Pressable
              ...
              accessibilityLabel={`View ${chair.title} on supplier site`}
            >
              <ThemedText
                type="small"
